...
const parseNumberWithPx = (v: number | undefined) => (v != null ? `${v}px` : undefined);
const parseLengthValue = (v: TLengthValue | undefined, defaultValue?: TLengthValue) => {
  if (v != null) return typeof v === 'number' ? parseNumberWithPx(v) : v;
  else return typeof defaultValue === 'number' ? parseNumberWithPx(defaultValue) : defaultValue;
};
